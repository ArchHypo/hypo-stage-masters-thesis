...
In this context, \emph{falsifiable} means that the possibility of proving the statement false exists. This means that the hypothesis is refutable and testable through evidence, and \emph{not} that the hypothesis must be phrased as something the team expects to be false.
ArchHypo prescribes that hypotheses be written as simple, direct statements that express the team's current beliefs, formulated freely to reduce the friction of adopting the technique \citep{silva2024tse}.
...
It is this hypothesis and uncertainty pairing that distinguishes the concept from adjacent concepts:
...
\begin{itemize}
  ...
  agrees to resolve the hypothesis only when a specific observable event occurs
...
Taken together, the background surveyed in this chapter frame architectural work as an
...
basis: each architectural hypothesis records one item of uncertainty as an explicit,
...
However, the empirical picture exposes a persistent limitation. The evaluations of
...
manual application, depended on expert team members, and offered no supporting tooling, leading
participants to find the technique hard to learn and its practice disconnected from
daily delivery. Additionally, the concept of Internal Developer Portals, and Backstage in particular
\citep{anjum2026idp,backstage2024}, suggested a single platform
for engineering workflows through which the techniques, such as ArchHypo, could be
operationalized. The Hypo-Stage prototype \citep{correa2025} took that first step, but it remained a feasibility demonstration rather than a workflow-ready tool.

The systematic mapping study \citep{souza2019} also confirms a
...
validated with real teams in production.

Grounded in these findings, this work takes Hypo-Stage as a starting artifact and asks how
real engineering teams appropriate ArchHypo in a production setting. Chapter~\ref{chap:related-work}
connects this agenda to the adjacent research on architectural decision-making, tool support, and hypotheses-driven practice.
