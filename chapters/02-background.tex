...
carried out in this thesis. It begins with a concise overview of software architecture and
architectural uncertainty in the context of scaled products, then introduces hypotheses
engineering and the ArchHypo technique in full technical detail, describes Internal
Developer Portals and their role as the delivery platform for tools such as Arch-Stage,
presents the Arch-Stage tool itself, and concludes with a summary of findings from
Souza et al.'s mapping study on the gap between architectural derivation methods and
...
with the characteristics it must support, the principles and decisions guiding its design,
and the design decisions themselves~\cite{richardsFord2020}. This definition emphasizes
that architecture is a living body of reasoning, not a frozen blueprint. Perry and Wolf's
earlier tripartite model~\cite{perryWolf1992} decomposed architecture into
\emph{elements} (the processing, data, and connecting constituents), \emph{form} (the
weighted relationships among those elements), and \emph{rationale} (the set of
constraints that justify the structure). Together, these accounts foreground the role of
uncertainty: the rationale behind any architectural choice is valid only relative to the
information available at the time of the decision, and that information is always
incomplete.
...
The concept of ``fast flow,'' popularized by the Team Topologies
framework~\cite{skelton2019}, emphasizes that organizations should design both their
...
without waiting for a central architect. This in turn demands that architectural
uncertainties be made explicit and that lightweight mechanisms exist for teams to
assess, prioritize, and revisit them as evidence accumulates.
...
conferences and through corporate training~\cite{silva2024ieeesw}, ArchHypo provides
...
An \emph{architectural hypothesis} is a falsifiable statement that captures an
...
\begin{itemize}
  ...
  is the epistemic gap, not the probability of an
  outcome~\cite{silva2024tse}.
...
assessments are made independently and relatively, not on absolute
probability~\cite{silva2024tse}.
...
technical plans accumulates. The responsible-moment principle—act when you have
enough information, neither too early nor too late—guides when a hypothesis should be
...
IEEE Software article by Silva et al.~\cite{silva2024ieeesw}. The setting was Catch
Solve, a startup offering web testing and monitoring services based in Italy's South
Tyrol region~\cite{silva2024ieeesw}. Four architectural hypotheses were formulated and
...
customers~\cite{silva2024ieeesw}. The study showed that naming and tracking
...
within a defined lifecycle~\cite{silva2024ieeesw}.
...
317~billion~BRL~\cite{silva2024tse}. The evaluation ran across six sprints of seven
...
\begin{enumerate}
  ...
  accommodates long-horizon uncertainties~\cite{silva2024tse}.

  \item ArchHypo enabled the team to avoid upfront architectural design, distributing
  structural decisions across iterations as evidence became available~\cite{silva2024tse}.

  \item Participants reported improvements to the development process in terms of
  safety, transparency, and predictability~\cite{silva2024tse}.
  ...
  work as not feeling directly connected to product delivery~\cite{silva2024tse}.
  ...
  continued expert support after the initial workshop~\cite{silva2024tse}.
\end{enumerate}

Findings~8 and~9 directly motivate the tooling contribution of this thesis: a tool that
embeds ArchHypo scaffolding in the daily engineering workflow, reducing the cognitive
overhead of manual application.
...
workflow of software engineers. In response, the industry has converged around the
...
into a unified ``pane of glass''~\cite{correa2025}. Its primary goal is to reduce
...
supported, and automated paths for building and deploying software~\cite{correa2025}.

This goal aligns directly with the theory of autonomous stream-aligned teams proposed
by Skelton and Pais~\cite{skelton2019}: if teams are to own full slices of a product
...
thesis. First, they embody the principle of \emph{decentralized architecture}: rather
...
engineering cadence—not a separate off-sprint activity—embedding it in the portal where
developers already spend their working time is strategically motivated.
...
more than 14,000 software components~\cite{backstage2024}. Spotify open-sourced the
framework in March~2020, and it was subsequently donated to the Cloud Native
Computing Foundation (CNCF) as an Incubating project~\cite{backstage2024}. As of
2025, more than 3,000 companies have adopted Backstage for their own
portals~\cite{backstage2024}.
...
infrastructure~\cite{correa2025}.
...
the appropriate delivery vehicle for Arch-Stage: a plugin can introduce a new class of
...
runs~\cite{correa2025}.

The undergraduate capstone project of Corrêa~\cite{correa2025}, supervised by the
author of this thesis, demonstrated the technical feasibility of building such a plugin
and validated the integration approach. That work established the initial version of
Arch-Stage, which constitutes the artifact under empirical evaluation in this thesis.
...
The case study in the capstone thesis reconstructed the Catch Solve startup scenario
...
tracking the evolution of assessments over time. These gaps motivate the empirical
evaluation and refinement effort in this thesis.
...
\begin{enumerate}
  ...
  automated or guided derivation path accessible to less-experienced
  practitioners~\cite{souza2019}.
  ...
  without senior architectural expertise had little actionable guidance~\cite{souza2019}.
  ...
  tracking within a team's daily workflow~\cite{souza2019}.
...
empirical evidence~\cite{souza2019}.

These findings reinforce the motivation for this thesis on two fronts. First, they confirm
...
empirical evaluations with real teams—precisely the kind of evaluation this thesis aims
to provide for Arch-Stage and ArchHypo.
