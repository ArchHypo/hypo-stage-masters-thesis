...
carried out in this present work. It begins with an overview of software architecture and
...
presents the Hypo-Stage tool itself, and concludes with a summary of findings from \cite{souza2019} who mapped the gaps between architectural derivation methods and practical tool support.

% ---------------------------------------------------------------------------
\section{Software Architecture and Architectural Uncertainty}
\label{sec:sw-arch}

Software architecture basically refers to the high-level structure of a software system, encompassing its components, their relationships, and the design principles that guide
its evolution ~\cite{bass2021}. However, in organizations that maintain software at the core of their business developed and operated by multiple teams,
...
but also the ability of engineering teams to work independently and deliver changes quickly and efficiently at a speed that needs to match the speed of the business ~\cite{skelton2019}.

More embracing to that rapidly changing business scenarios, \cite{richardsFord2020} added that the design decisions themselves are part of the software architecture itself, 
...
This work sees that modern definition as a way of forging the role of Architectural Uncertainty: the rationale behind any architectural choice is valid only relative to the information available at the time of the decision, 
...
quality over time, turning the software architecture into a big ball of mud as perceived by ~\cite{footeYoder1999} in the late 90s.

Moreover, the concept of ``fast flow'', popularized by the Team Topologies
framework in ~\cite{skelton2019}, emphasizes that organizations should design both their
...
This is where Hypotheses Engineering may be used to play a key role in the architecture work, as proposed by \cite{silva2024tse}.
...
architectural decisions as testable hypotheses rather than settled conceived facts. Instead of
...
The ArchHypo technique, proposed in ~\cite{silva2024tse}, operationalized this idea for
...
conferences and through corporate training reported in~\cite{silva2024ieeesw}, ArchHypo provides
...
\begin{itemize}
  ...
  The distinguishing feature is the epistemic gap, not the probability of an outcome as observed in ~\cite{silva2024tse}.
...
assessments are made independently and relatively, not on absolute probability as reported in ~\cite{silva2024tse}.
...
deferred. ArchHypo identifies the following plan strategies as observerd in ~\cite{silva2024ieeesw}:
...
\begin{itemize}
  ...
  normal work. This was the most frequently used strategy in the evaluation reported in~\cite{silva2024tse}.
...
technical plans accumulates. The responsible-moment is the principle—act of the team during the hypothesis lifecycle. 
It is when they have enough information, neither too early nor too late, to take the effort to move forward with the architectural decision or defer it further.
...
IEEE Software article in ~\cite{silva2024ieeesw}. The setting was Catch
...
\begin{enumerate}
  ...
  One participant explicitly stated that *''more training would make the team less dependent on the team of architects''* (P3) ~\cite{silva2024tse}. 
...
Findings ~8 and ~9 directly motivate the tooling contribution of this present work: a tool that
...
\midrule
\citet{silva2024ieeesw} & Catch Solve startup (Italy) & 4 hypotheses, 10 months & Explicit hypothesis tracking changed how team prioritized architectural investments. & Single practitioner; no tool support. \\
\midrule
\citet{silva2024tse} & PropSEP / Jetsoft (Brazil) & 10 hypotheses, 6 sprints, 8 participants & ArchHypo distributed architectural work across iterations; all participants found technique hard to learn. & Manual application; expert steering required; no supporting tool. \\
\midrule
This work & Large-scale tech organization (Latin America) & Multiple teams, scaled product & To be reported --- first evaluation in a tool-mediated setting via Arch-Stage. & Single organizational context; Backstage dependency. \\
\bottomrule
\end{tabular}
\end{table}
...
by ~\cite{skelton2019}: if teams are to own full slices of a product
...
\toprule
\textbf{Layer} & \textbf{Responsibilities} & \textbf{Relevance to Arch-Stage} \\
\midrule
Core Framework & Plugin loader, routing, authentication, error handling. & Provides the lifecycle hooks that Arch-Stage's frontend and backend plugins register into. \\
\midrule
Plugin Ecosystem & Frontend plugins (views, UI); backend plugins (API services, data, storage). & Arch-Stage is implemented as a split plugin: the frontend renders hypotheses and assessments; the backend persists data and exposes the API. \\
\midrule
External Integrations & Backend plugins as gateways to external systems (cloud, CI/CD, APIs). & Arch-Stage integrates with the Backstage catalog to auto-associate hypotheses with software entities. \\
\bottomrule
\end{tabular}
\end{table}
...
~\cite{correa2025} demonstrated the technical feasibility of building such a plugin
and validated the integration approach. That work established the initial version of
the Hypo-Stage plugin for Backstage, which constitutes the artifact under empirical evaluation in this present work.
...
the author of this present work and co-supervised by Prof.\ Paulo
Meirelles ~\cite{correa2025}. Its purpose is to operationalize ArchHypo within the Spotify's IDP Backstage, embedding hypothesis management into the engineering platform workflow.
...
The case study by ~\cite{correa2025} reconstructed the Catch Solve startup scenario
...
evaluation and refinement effort promoted in this present work.
...
The systematic mapping study by ~\cite{souza2019} examined
...
\begin{enumerate}
  ...
  automated or guided derivation path accessible to less-experienced practitioners~\cite{souza2019}.

  \item \textbf{Limited support for less-experienced practitioners.} Methods assumed a
  skilled architect capable of navigating trade-offs among quality attributes. Teams without senior architectural expertise had little actionable guidance~\cite{souza2019}.
...
~\cite{souza2019} \ also noted that 76.9\% of the 39 primary studies addressed functional
