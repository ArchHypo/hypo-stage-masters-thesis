...
Hypotheses engineering is a general approach to managing uncertainty in software
development, first articulated by \citet{melegati2019he}. Its core idea is to treat
uncertain decisions as testable hypotheses rather than settled facts: instead of
asserting that a particular choice \emph{will} produce a desired outcome, teams state
that it \emph{might} do so and then design experiments to gather evidence that
validates or invalidates the hypothesis. Although it originated in the broader context
of software startups and product development under uncertainty, the same discipline of
making assumptions explicit and evidence-driven can be applied wherever teams must
act despite incomplete information.

The ArchHypo technique, proposed by \citet{silva2024tse}, is the architecture-specific
adaptation of hypotheses engineering: it operationalizes this idea for software
architecture, where a particular technology or pattern is treated as a hypothesis about
whether it will satisfy a quality requirement rather than an assumed fact. Grounded in
Design Science Research (DSR) and evolved from a body of patterns for agile architecture
first disseminated at conferences and through corporate training
\citep{silva2024patterns,silva2024ieeesw}, ArchHypo provides a structured vocabulary and
a set of practices that fit within iterative development workflows where architecture
uncertainty is a constant challenge for rapidly-evolving software systems.

% ------- 2.2.1
\subsection{Core Concepts}
\label{subsec:archhypo-core}

An \emph{architectural hypothesis} is a \emph{falsifiable} statement that captures an uncertainty relevant to the design of a software architecture.
Here, \emph{falsifiable} means that the possibility of proving the statement false exists---the hypothesis is refutable and testable through evidence---and \emph{not} that the hypothesis must be phrased as something the team expects to be false.
ArchHypo prescribes that hypotheses be written as simple, direct statements that express the development team's current beliefs, formulated freely to reduce the friction of adopting the technique \citep{silva2024tse}.
The practice descends from Hypotheses Engineering, first articulated by \citet{melegati2019he}; interrogative or questioning templates such as QUESt \citep{melegati2019quest} are an optional, compatible way to represent hypotheses rather than a requirement of the technique.
Equating ``falsifiable'' with ``must be phrased as expected to be false'' is therefore a misreading of the prescription.
In this sense, an architectural hypothesis is fundamentally \emph{a way of recording and registering an item of architectural uncertainty}: each hypothesis pairs one uncertainty with an explicit, testable statement about it, turning otherwise implicit assumptions into first-class artifacts that can be assessed, tracked, and revisited as evidence accumulates \citep{silva2024tse}.
It is this hypothesis\,$\leftrightarrow$\,uncertainty pairing that distinguishes the concept from adjacent concepts:
...
\begin{itemize}
  ...
  agrees to resolve the hypothesis---that is, to take or deliberately defer the
  associated architectural decision---only when a specific observable event occurs
  (e.g., when the number of concurrent users exceeds a threshold, or when a performance
  regression appears in monitoring). Until the trigger fires, the decision is
  deliberately deferred rather than made prematurely.
...
\paragraph{Start-up evaluation (Catch Solve).}
...
\paragraph{Mission-critical evaluation (PropSEP at Jetsoft).}
...
\begin{enumerate}
  ...
  One participant explicitly stated that \emph{``more training would make the team less dependent on the team of architects''} (P3) ~\citep{silva2024tse}. 
...
independence ~\citep{anjum2026idp}.
...
into a unified ``pane of glass'' ~\citep{anjum2026idp}. Its primary goal is to reduce
...
supported, and automated paths for building and deploying software ~\citep{anjum2026idp}.
...
infrastructure ~\citep{backstage2024}.

\begin{table}[htbp]
\centering
\caption{Backstage architecture layers and their role \citep{backstage2024}.}
...
Figure~\ref{fig:initial-hypostage-dashboard} shows the main hypotheses dashboard of that initial version.

\begin{figure}[ht]
  \centering
  \includegraphics[width=0.95\textwidth]{initial-hypostage-dashboard}
  \caption{Initial Hypo-Stage hypotheses dashboard (Backstage plugin), corresponding to the
    \citealt{correa2025} baseline at commit~\texttt{ceee509}. The listing exposes each
    hypothesis with uncertainty, impact, status, source type, and creation date.}
  \label{fig:initial-hypostage-dashboard}
\end{figure}
...
The systematic mapping study \citep{souza2019} examined methods for deriving
architectural models from requirements specifications, analyzing 39 high-quality primary
studies selected from a broader literature search.
...
These findings characterize the state of architectural derivation methods at the time of
the mapping: reliant on tacit expertise, weakly tooled, and thinly evaluated in
production settings.

% ---------------------------------------------------------------------------
\section{Summary: State of the Literature for This Work}
\label{sec:background-summary}

Taken together, the strands surveyed in this chapter frame architectural work as an
exercise in managing uncertainty. Hypotheses Engineering \citep{melegati2019he} and its
architecture-specific realization, ArchHypo \citep{silva2024tse}, provide the conceptual
machinery: each architectural hypothesis records one item of uncertainty as an explicit,
testable statement, which is then assessed for epistemic gap and priority and worked
through a technical plan until enough evidence accumulates to resolve it. This turns
otherwise implicit assumptions into first-class artifacts that a team can track and
revisit as a project evolves.

The empirical picture, however, exposes a persistent limitation. The evaluations of
ArchHypo reported to date \citep{silva2024ieeesw,silva2024tse} showed real value in
distributing architectural reasoning across iterations, but they were conducted through
manual application, depended on expert steering, and offered no supporting tool—leaving
participants finding the technique hard to learn and its bookkeeping disconnected from
daily delivery. Internal Developer Portals, and Backstage in particular
\citep{anjum2026idp,backstage2024}, suggest a natural delivery vehicle: a single platform
already woven into the engineering workflow through which such a technique could be
operationalized. The Hypo-Stage prototype \citep{correa2025} took a first step in that
direction but remained a feasibility demonstration rather than a workflow-ready tool.

This gap is not incidental. The systematic mapping study \citep{souza2019} confirms a
broader ``tooling void'' in which methods for deriving and managing architecture remain
disconnected from the daily engineering workflow, weakly supported by tools, and rarely
validated with real teams in production. The opening this thesis addresses follows
directly: to close the distance between a promising but manual technique and the platform
where engineering work actually happens, and to subject the result to the kind of
empirical evaluation the literature has lacked. The next chapter situates this
contribution against related efforts and positions it within the surrounding body of
work.
