...
Teams are expected to deliver more features in shorter cycles while preserving scalability, performance, and other quality attributes of their systems that continue to grow in size and complexity, as observed in \citet{silva2024tse,silva2024ieeesw}. 
...
Uncertainty in architecture decisions proceeds from limited knowledge about requirements, future usage scenarios, and the properties of candidate solutions as reviewed in \citet{silva2024tse}. 
...
A complementary perspective is to acknowledge uncertainty explicitly and as an inevitable aspect of the architecture work, and therefore, to manage it as a first-class concern during the architecture work as proposed by \citet{silva2024tse}.

In this regard, Hypotheses Engineering has emerged as an approach to manage uncertainty in software development by treating assumptions as explicit, falsifiable hypotheses that can be prioritized and tested through experiments \citep{melegati2019he,olsson2014hypex,fagerholm2017,silva2024tse,silva2024ieeesw}. In this context, ArchHypo is the technique that adapts hypotheses engineering to software architecture. 
...
At the same time, both studies report that the technique introduces a non-trivial learning curve and requires adjustments to existing processes, which might pose a challenge to its adoption by the team as reported in \citet{silva2024tse,silva2024ieeesw}.
...
Many methods describe high-level processes or frameworks, but offer little guidance on how to operationalize them in everyday practice as reported in \citet{souza2019}.

The same study highlighted gaps in tool support. A significant portion of the methods did not include any supporting tool, and when tools were present, they tended to address only parts of the process as also reported in \citet{souza2019}. 
...
Grounded in these studies' contributions, an initial version of Hypo-Stage --- a support tool designed to help teams apply the ArchHypo technique in practice --- was developed as prior work \citep{correa2025}; this present work takes that tool as its starting artifact rather than as one of its contributions.
...
\begin{itemize}
  \item Methods for deriving and evolving architectures from requirements still depend strongly on architects' tacit knowledge and lack comprehensive tool support as reported in \citet{souza2019}.
  \item ArchHypo offers a promising structure for handling architectural uncertainty, but teams perceive the technique as hard to learn and requiring process changes, especially without specialized support as reported in \citet{silva2024tse,silva2024ieeesw}.
  \item Hypo-Stage provides an initial implementation of tool support for ArchHypo, but its impact on real software engineering teams and the refinements needed for effective use in industry remain unknown as reported in \citet{correa2025}.
\end{itemize}
...
Investigating these concerns can contribute both to the practical adoption of ArchHypo via Hypo-Stage and to the broader discussion on how to support teams in sharing architectural responsibilities for accelerating adoption of the socio-technical architecture paradigm mapped in \citet{infoq2024trends}.
...
To achieve the above objectives, this work is guided by four research questions, presented in the same order in which they are addressed in Chapter~\ref{chap:results}. Their mapping to the objectives is summarized in Table~\ref{tab:rq-objectives}.
...
By answering its set of research questions, this work makes the following contributions:
...
\begin{itemize}
  ...
  \item A set of novel \textit{Structural Hypothesis Patterns} derived from thematic analysis of the hypotheses registered during the study. These patterns --- Hypothesis Cluster, Regulatory Trigger Hypothesis, Service Decomposition Hypothesis, Monitored Quality-Attribute Hypothesis, and Shared-Resource Hypothesis --- constitute the first empirically grounded answer to the question of what recurring shapes architectural hypotheses take when ArchHypo is applied in practice, addressing a gap explicitly identified as future work in the prior ArchHypo pattern literature \citep{silva2024patterns}.
  \item A set of empirically grounded refinements to the Hypo-Stage tool and its usage guidelines, aimed at improveing the tool's usability and engagement, and better integrating the technique into team workflows as reported in \citet{correa2025}.
  \item Reflections on the implications of these findings for supporting distributed architectural work in agile and continuous development settings, connecting the ArchHypo experience to broader concerns about tacit knowledge and tool support in software architecture as reported in \citet{souza2019,infoq2024trends}.
\end{itemize}
...
adoption-condition candidate \emph{Registration as a Slack-Dependent Practice}
(A3, Section~\ref{sec:group-a-patterns}).
...
\textit{Shared-Resource Hypothesis}) that are novel contributions
...
assessment changes over time. These findings are previewed here to give the reader an early overview. A complete result analysis and
...
recorded, including aggregate data characterization and the Structural Hypothesis
Patterns --- the recurring structural shapes that hypotheses take, developed in
Chapter~\ref{chap:results} (RQ2); the benefits and challenges they perceived (RQ3); and the refinements to
