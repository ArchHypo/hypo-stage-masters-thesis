...
for the RQ3 themes appears in Table~\ref{tab:code-traceability}
(\textit{tool-positive-catalog-integration},
\textit{registration-barrier-time-pressure}, and
\textit{falsifiability-confusion}).
...
unknown.
The related open code
\textit{tool-positive-catalog-integration} captures participants' positive
appraisal of surfacing Hypo-Stage within their existing Backstage catalog
workflow, which Section~\ref{subsec:refinement-backstage} later operationalizes
as a refinement.
...
(A1--A4) are \emph{candidate conditions} --- empirically grounded candidate
propositions about the conditions under which hypothesis engineering is adopted
--- and \emph{not} validated patterns. Unlike a pattern, which pairs a recurring
...
to the future-work research agenda (Section~\ref{sec:future-work}), which
...
Both hypotheses were validated through a Guideline technical plan — an alignment
session held before the observation window and registered in Hypo-Stage retrospectively —
that produced a governance decision enabling
...
Adoption-Condition Candidates (A1--A4; empirically grounded candidate conditions
for sustainable hypothesis engineering, \emph{not} validated patterns) and the
five Structural Hypothesis Patterns (B1--B5; structural archetypes of
architectural hypotheses). Both families describe problem and context only;
solution-shaped guidance for each item is deferred to the future-work research
